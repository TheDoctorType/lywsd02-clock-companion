...
$SyncScript = Join-Path $ScriptDir 'Sync-LYWSD02.ps1'
if (-not $SettingsPath) { $SettingsPath = Join-Path $ScriptDir 'settings.json' }
$LogDir   = Join-Path $ScriptDir 'logs'
$CsvPath  = Join-Path $LogDir 'sensors.csv'
...
function Reading-Text {
    if ($script:lastReading) {
        ...
        return ("{0:0.0} C   {1}% RH   dew {2:0.0} C   batt {3}   @ {4}" -f $tc,$hu,$dew,$batTxt,$script:lastReading.When.ToString('HH:mm'))
    }
    return 'LYWSD02 - no reading yet'
}
...
$miTitle   = New-Item 'LYWSD02 Clock'; $miTitle.Enabled = $false; $f=$miTitle.Font; $miTitle.Font = New-Object System.Drawing.Font($f.FontFamily,$f.Size,[System.Drawing.FontStyle]::Bold)
$miReading = New-Item 'No reading yet'; $miReading.Enabled = $false
$miRead    = New-Item 'Read now'
...
$menu.Items.AddRange(@(
    ...
    $miRead, $miSync, $miTrends,
    (New-Object System.Windows.Forms.ToolStripSeparator),
    $miConn, $miLog, $miOpen, $miStartup,
    (New-Object System.Windows.Forms.ToolStripSeparator),
    $miExit
))
...
function Refresh-Menu {
    ...
    $busy = ($null -ne $script:job)
    $miRead.Enabled = (-not $busy) -and $script:settings.ConnectionEnabled
    $miSync.Enabled = (-not $busy) -and $script:settings.ConnectionEnabled
}

# ---- Async BLE jobs (delegated to Sync-LYWSD02.ps1) -----------------------
function Start-Bg {
    ...
    try {
        ...
        $script:job = @{ Proc=$p; Out=$tmp; Kind=$Kind; LogCsv=$LogCsv; Notify=$Notify }
        if ($Kind -eq 'read') { Update-Icon '..' (-not $script:settings.ConnectionEnabled) }
        $script:ni.Text = "LYWSD02  -  $Kind in progress..."
        $script:poll.Start()
        Refresh-Menu
    } catch {
        WLog "ERROR launching $Kind : $($_.Exception.Message)"
        $script:job = $null
...
function Complete-Job {
    $j = $script:job
    if ($null -eq $j) { $script:poll.Stop(); return }
    if (-not $j.Proc.HasExited) { return }
    $script:poll.Stop()
    $script:job = $null
    $raw = ''
    ...
    $data = $null
    if ($line) { try { $data = $line | ConvertFrom-Json } catch {} }
    ...
    if ($j.Kind -eq 'read') {
        if ($data -and $data.ok) {
            ...
            $batTxt = if ($null -eq $bat) { '?' } else { "$bat%" }
            $dew = Get-Dewpoint $tc $hu
            $miReading.Text = ("{0:0.0}C   {1}%   dew {2:0.0}C   batt {3}   @ {4}" -f $tc, $hu, $dew, $batTxt, (Get-Date -Format 'HH:mm'))
            $script:ni.Text = ("LYWSD02  {0:0.0}C  {1}%RH  dew {2:0.0}C  batt {3}" -f $tc, $hu, $dew, $batTxt)
            ...
            if ($j.Notify) { $script:ni.ShowBalloonTip(4000,'LYWSD02', ("{0:0.0} C   {1}% RH   battery {2}" -f $tc,$hu,$batTxt), [System.Windows.Forms.ToolTipIcon]::Info) }
        } else {
            Update-Icon '--' (-not $script:settings.ConnectionEnabled)
            $script:ni.Text = 'LYWSD02  -  last read failed'
            if ($j.Notify) { $script:ni.ShowBalloonTip(4000,'LYWSD02','Could not read the clock (not seen / out of range).',[System.Windows.Forms.ToolTipIcon]::Warning) }
            WLog "WARN read failed: $line"
        }
    } else { # sync
        if ($data -and $data.ok) {
            if ($j.Notify) { $script:ni.ShowBalloonTip(4000,'LYWSD02','Clock time synchronised.',[System.Windows.Forms.ToolTipIcon]::Info) }
            WLog "INFO clock synced"
        } else {
            if ($j.Notify) { $script:ni.ShowBalloonTip(4000,'LYWSD02','Clock sync failed (not seen / out of range).',[System.Windows.Forms.ToolTipIcon]::Warning) }
            WLog "WARN sync failed: $line"
        }
    }
    # Refresh any open graph views with the new data.
    if ($script:popupShown) { try { $script:popupHdr.Text = Reading-Text; Rebuild-ChartData $script:popupChart '' } catch {} }
    if ($script:trendForm -and -not $script:trendForm.IsDisposed) { try { Rebuild-ChartData $script:trendChart ("LYWSD02   -   " + (Reading-Text)) } catch {} }
    Refresh-Menu
}
...
$script:poll.Add_Tick({ Complete-Job })

$script:hour = New-Object System.Windows.Forms.Timer
$script:hour.Interval = [int]([math]::Max(1,[int]$script:settings.IntervalMinutes) * 60 * 1000)
$script:hour.Add_Tick({ if ($script:settings.ConnectionEnabled) { Start-Bg -Kind 'read' -LogCsv ([bool]$script:settings.HourlyLogging) -Notify $false } })
if ($script:settings.ConnectionEnabled) { $script:hour.Start() }

# One-shot kickoff read shortly after launch so the icon populates.
$script:kick = New-Object System.Windows.Forms.Timer
$script:kick.Interval = 3000
$script:kick.Add_Tick({
    $script:kick.Stop()
    if ($script:settings.ConnectionEnabled) { Start-Bg -Kind 'read' -LogCsv ([bool]$script:settings.HourlyLogging) -Notify $false }
})
$script:kick.Start()
...
function Show-TrendsWindow {
    try {
        ...
        $f.Text = 'LYWSD02  -  Temperature / Humidity / Dew point'
        $f.Size = New-Object System.Drawing.Size(900,560)
        $f.MinimumSize = New-Object System.Drawing.Size(560,360)
        $f.StartPosition = [System.Windows.Forms.FormStartPosition]::CenterScreen
        $f.BackColor = [System.Drawing.Color]::FromArgb(24,24,28)
        $fg = [System.Drawing.Color]::FromArgb(225,225,225)
        $chart = New-TrendChart $false
        ...
        $applyRange = {
            try {
                ...
                Save-Settings
                if ($script:trendChart) { Rebuild-ChartData $script:trendChart ("LYWSD02   -   " + (Reading-Text)) }
        ...
        $btnRefresh.Location=New-Object System.Drawing.Point(($cw-176),6)
        $btnCsv = New-Object System.Windows.Forms.Button
        $btnCsv.Text='Open CSV'; $btnCsv.Width=86; $btnCsv.ForeColor=$fg; $btnCsv.FlatStyle=[System.Windows.Forms.FlatStyle]::Flat
        $btnCsv.Anchor=([System.Windows.Forms.AnchorStyles]::Top -bor [System.Windows.Forms.AnchorStyles]::Right)
        $btnCsv.Location=New-Object System.Drawing.Point(($cw-90),6)
        $btnRefresh.Add_Click({ Rebuild-ChartData $script:trendChart ("LYWSD02   -   " + (Reading-Text)) })
        $btnCsv.Add_Click({ if (Test-Path $CsvPath) { Start-Process $CsvPath } })

        $bar.Controls.AddRange(@($lblRange,$script:cbRange,$script:dtFrom,$lblTo,$script:dtTo,$btnRefresh,$btnCsv))
        $f.Controls.Add($chart)   # Fill added first
        $f.Controls.Add($bar)     # Bottom added last
        $script:trendForm = $f; $script:trendChart = $chart
        Rebuild-ChartData $chart ("LYWSD02   -   " + (Reading-Text))
        $f.Add_FormClosed({ $script:trendForm = $null; $script:trendChart = $null })
        $f.Show(); $f.Activate()
...
$miConn.Add_Click({
    ...
    if ($script:settings.ConnectionEnabled) {
        ...
        Start-Bg -Kind 'read' -LogCsv ([bool]$script:settings.HourlyLogging) -Notify $false
